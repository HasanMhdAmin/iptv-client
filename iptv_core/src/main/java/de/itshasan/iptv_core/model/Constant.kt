package de.itshasan.iptv_core.model

object Constant {
    private const val ROOT = "de.itshasan.iptv_core."
    const val CATEGORY_ID = ROOT + "CATEGORY_ID"
    const val ALL_SERIES = ROOT + "ALL_SERIES"
    const val ALL_MOVIES = ROOT + "ALL_MOVIES"
    const val CONTENT_ID = ROOT + "CONTENT_ID"
    const val CONTENT = ROOT + "CONTENT"
    const val COVER_URL = ROOT + "COVER_URL"
    const val SERIES_TITLE = ROOT + "SERIES_TITLE"
    const val CURRENT_TIME = ROOT + "CURRENT_TIME"
    const val ALL_EPISODES = ROOT + "ALL_EPISODES"
    const val TYPE_SERIES = "SERIES"
    const val TYPE_MOVIES = "MOVIES"
    const val TARGET = "TARGET"

}